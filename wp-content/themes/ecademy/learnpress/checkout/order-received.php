...
if ( isset( $order ) ) {
	...
	?>
    <p><?php echo apply_filters( 'learn-press/order/received-order-message', __( 'Thank you. Your order has been received.', 'ecademy' ), $order ); ?></p>

    <div class="table-responsive">
		<table class="order_details">
			<tr class="order">
				<th><?php esc_html_e( 'Order Number', 'ecademy' ); ?></th>
				<td>
					<?php echo wp_kses_post( $order->get_order_number() ); ?>
				</td>
			</tr>
			<tr class="item">
				<th><?php esc_html_e( 'Item', 'ecademy' ); ?></th>
				<td>
					<?php
					$links = array();
					...
					foreach ( $items as $item ) {
						if ( empty( $item['course_id'] ) || get_post_type( $item['course_id'] ) !== LP_COURSE_CPT ) {
							$links[] = __( 'Course does not exist', 'ecademy' );
						} else {
			...
			<?php if ( $method_title = $order->get_payment_method_title() ) : ?>
				<tr class="method">
					<th><?php esc_html_e( 'Payment Method', 'ecademy' ); ?></th>
					<td>
...
<?php } else { ?>
    ...
    <p><?php echo apply_filters( 'learn-press/order/received-invalid-order-message', __( 'Invalid order.', 'ecademy' ) ); ?></p>
