...
if ( isset( $drip_item['open_item'] ) ) {
	...
	if ( $drip_item['depends']['type'] != 'immediately' ) {
		$message .= $drip_item['depends']['date'] ? __( ' and ', 'ecademy-toolkit' ) . ( $drip_item['depends']['type'] == 'interval' ? __( 'after ', 'ecademy-toolkit' ) . $drip_item['depends']['interval'][0] . ' ' . $type_text . __( '(s)', 'ecademy-toolkit' ) : __( 'on ', 'ecademy-toolkit' ) . date( get_option( 'date_format' ) . ' H:i', $drip_item['depends']['date'] ) ) : '';
	}
