...
class ECA_Woo_Payment extends LP_Addon {
	/**
	 * @var string
	 */
	public $version = ECA_WOO_PAYMENT_VER;

	/**
	 * @var string
	 */
	public $require_version = ECA_WOO_PAYMENT_REQUIRE_VER;
	 ...
	 */
	protected function _inc() {
		// WooCommerce activated
		if ( $this->woo_actived() && function_exists( 'LP' ) ) {
			require_once ECA_WOO_PAYMENT_PATH . '/inc/functions.php';
			require_once ECA_WOO_PAYMENT_PATH . '/inc/admin/course.php';
			// Enabled payment and checkout
			if ( $this->is_enabled() && $this->woo_payment_enabled() || $this->woo_checkout_enabled() ) {
				require_once ECA_WOO_PAYMENT_PATH . '/inc/class-wc-product-lp-course.php';
			}
			// init hooks
			$this->init_hooks();
			$payment = ECA_WOO_PAYMENT_PATH . '/inc/class-lp-wc-payment.php';
			...
			if ( $this->is_enabled() && $this->woo_checkout_enabled() ) {
				...
				$checkout = require_once ECA_WOO_PAYMENT_PATH . '/inc/class-lp-wc-checkout.php';
	...
	public function init_hooks() {
		...
		add_filter( 'woocommerce_add_to_cart_validation', array(
			$this,
			'woocommerce_add_to_cart_course_validation'
		), 100, 3 );
	}
	...
	public function wc_order_notice() {
        ...
        <div class="error woo-payment-order-notice">
            <p>
				<?php printf( __( 'This order is related to LearnPress order, so if you want to do anything with LearnPres please edit it <a href="%s">here</a>', 'ecademy-toolkit' ), get_edit_post_link( $lp_order_id ) ); ?>
            </p>
        </div>
		<?php
	}
	...
	public function scripts() {
		if ( function_exists( 'learn_press_assets' ) ) {
	...
	public function learnpress_update_order_status( $order_id, $old_status, $new_status ) {
		...
		if ( $lp_order_id && get_post_type( $lp_order_id ) === LP_ORDER_CPT ) {
			...
			if ( $lp_order ) {
				...
				/**
				 * Ensure get newest status of wc order has already updated in db.
				 * The $new_status may not sync in the series of hooks related to
				 * status of wc order.
				 */
				$post_status = get_post_status( $order_id );
				$new_status  = 'wc-' === substr( $post_status, 0, 3 ) ? substr( $post_status, 3 ) : $post_status;
	...
	public static function admin_notice() {
		switch ( self::$_error ) {
			...
			case 2:
				?>
                <div class="error">
                    <p><?php printf( __( '<strong>eCademy Toolkit</strong> version %s requires <strong>LearnPress</strong> version %s or higher', 'ecademy-toolkit' ), ECA_WOO_PAYMENT_VER, ECA_WOO_PAYMENT_REQUIRE_VER ); ?></p>
                </div>
				<?php
				break;
			case 3:
				?>
                <div class="error">
                    <p><?php echo __( '<strong>eCademy</strong> Toolkit for LearnPress is not compatible with WooCommerce 3.0.0. Please temporary deactivate it. We are working for an update. We will try to finish it soon.', 'ecademy-toolkit' ); ?></p>
                </div>
			<?php
		}
	}
