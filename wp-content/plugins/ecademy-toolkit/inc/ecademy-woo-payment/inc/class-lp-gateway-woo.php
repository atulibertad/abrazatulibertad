...
class LP_Gateway_Woo extends LP_Gateway_Abstract {
	...
	public function __construct() {
		...
		$this->method_description = __( 'Make a payment with WooCommerce payment.', 'ecademy-toolkit' );
	...
	public function get_settings() {
		...
		if ( $payment_gateways ) {
			foreach ( $payment_gateways as $payment_gateway ) {
				$title = sprintf(
					__( 'This payment is %s, please click the link beside to enable/disable.', 'ecademy-toolkit' ),
		...
		return
			array(
				array(
					...
					'desc'    => __( 'If enable this then you can not use other payments provided by LearnPress', 'ecademy-toolkit' )
				),
				array(
					'title'   => __( 'Purchase button', 'ecademy-toolkit' ),
					...
					'options' => array(
						'single' => __( 'Enable On Single button', 'ecademy-toolkit' ),
					)
				),
			);
	}
	...
	public function is_available( $available, $gateway ) {
		return ECA_Woo_Payment::instance()->is_enabled() && ECA_Woo_Payment::instance()->woo_payment_enabled() && sizeof( WC()->payment_gateways()->get_available_payment_gateways() );
	}
